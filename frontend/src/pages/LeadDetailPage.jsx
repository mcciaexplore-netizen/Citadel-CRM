...
export default function LeadDetailPage() {
    ...
    const [ordForm, setOrdForm] = useState({
        ProductOrdered: lead ? lead['ProductRequired (AAC Blocks/Citabond Mortar/Kavach Plaster)']?.split(',')[0] : '',
        OrderQuantity: '',
        Unit: 'Cubic Meters',
    ...
    const handleOrdSubmit = async (e) => {
        ...
        try {
            await createOrder({
                ...
                ProductOrdered: ordForm.ProductOrdered,
                OrderQuantity: `${ordForm.OrderQuantity} ${ordForm.Unit}`,
                DispatchDate: ordForm.DispatchSchedule,
                DispatchSchedule: ordForm.DispatchSchedule,
                AssignedUserID: lead.AssignedUserID
            });
            showToast('Order created & Thank You email sent!');
        ...
        <div className="space-y-6 pb-12">
                        ...
                        <div className="bg-gray-50 p-5 rounded-lg border border-gray-100 flex flex-col justify-center space-y-4">
                            ...
                            {lead.OrderFlag === 'Y' && (
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded border border-green-200 text-xs">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="p-6 md:p-8 min-h-[400px]">
                                            ...
                                            <div className="bg-white border border-gray-100 p-5 rounded-xl shadow-sm group-hover:shadow-md transition-shadow">
                                                ...
                                                <div className="mt-4 pt-4 border-t border-gray-50 flex flex-wrap justify-between items-center gap-4 text-xs">
                                                    {int.NextFollowUpDate ? (
                                                        <span className="flex items-center gap-1.5 font-bold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full border border-orange-100">
                    ...
                    ${quotation['ApprovalStatus (NotRequired/PendingApproval/Approved/Rejected)'] === 'PendingApproval' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                                            quotation['ApprovalStatus (NotRequired/PendingApproval/Approved/Rejected)'] === 'Approved' ? 'bg-green-50 text-green-700 border-green-200' :
                            ...
                            <form onSubmit={handleQuoSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" step="0.01" value={quoForm.QuotedPricePerUnit} onChange={e => setQuoForm({ ...quoForm, QuotedPricePerUnit: e.target.value })} className={`w-full px-3 py-2 border rounded-md focus:ring-primary focus:border-primary ${parseFloat(quoForm.QuotedPricePerUnit) < thresholdPrice ? 'border-orange-400 bg-orange-50' : 'border-gray-300'}`} placeholder="e.g. 3500" />
                                        {parseFloat(quoForm.QuotedPricePerUnit) < thresholdPrice && (
                                            <p className="text-xs text-orange-600 font-bold mt-1">Requires Admin Approval (&lt; ₹{thresholdPrice})</p>
                                        )}
                                    </div>
                                    <div>
                                ...
                                <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
                                    ...
                                    {isAdmin && quotation && quotation['ApprovalStatus (NotRequired/PendingApproval/Approved/Rejected)'] === 'PendingApproval' && (
                                        <div className="flex gap-2">
                                            <button type="button" onClick={async () => { await approveQuotation(quotation.QuotationID); fetchData(); showToast('Approved!'); }} className="bg-green-100 text-green-800 px-4 py-2 rounded font-bold hover:bg-green-200 transition-colors">Approve</button>
                        ...
                        <div className="animate-in fade-in duration-300 max-w-3xl">
                            {!order ? (
                                <div className="text-center py-16 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                    ...
                                    <button onClick={() => setIsOrderModalOpen(true)} className="bg-green-600 text-white font-bold px-6 py-3 rounded-lg shadow flex items-center gap-2 mx-auto hover:bg-green-700 transition-colors">
                                ...
                                <div className="bg-white rounded-xl border border-green-200 overflow-hidden shadow-sm">
                                    <div className="bg-green-50 px-6 py-4 border-b border-green-100 flex justify-between items-center">
                                        <div>
                                            <h3 className="text-lg font-bold text-green-900">Order Placed</h3>
                                            <p className="text-sm font-mono text-green-700 mt-0.5">ID: {order.OrderID}</p>
                                        </div>
                                        <span className="bg-green-600 text-white font-bold px-3 py-1 rounded-full text-sm shadow-sm uppercase tracking-wide flex items-center gap-1.5">
                                    ...
                                    <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                                        <div>
                                            <p className="text-sm text-gray-500 font-medium mb-1">Product Ordered</p>
                                            <p className="font-bold text-gray-900 text-lg">{order.ProductOrdered}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-500 font-medium mb-1">Quantity</p>
                                            <p className="font-bold text-gray-900 text-lg">{order.OrderQuantity}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div className="sm:col-span-2 pt-4 border-t border-gray-100 flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {order['ThankyouEmailSent (Yes/No)'] === 'Yes' ? (
                                                    <span className="flex items-center gap-1 bg-green-100 text-green-800 text-xs font-bold px-2 py-0.5 rounded"><CheckCircle2 size={12} /> Sent</span>
                                                ) : (
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <select
                                                    ...
                                                    className="px-3 py-1.5 border border-green-300 rounded font-bold text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
                                                >
                    ...
                    {activeTab === 'payment' && (
                      ...
                      ${payment['PaymentStatus (Pending/Partial/Paid)'] === 'Paid' ? 'bg-green-100 text-green-800 border-green-300' :
                                                payment['PaymentStatus (Pending/Partial/Paid)'] === 'Partial' ? 'bg-orange-100 text-orange-800 border-orange-300' :
                                    ...
                                    <div className="p-6 grid grid-cols-1 sm:grid-cols-3 gap-6">
                                        ...
                                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                                            ...
                                            <p className="font-bold text-green-600 text-2xl">₹{payment.PaidAmount}</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <span className={`text-xs font-bold px-2 py-1 rounded inline-block mt-0.5 ${payment['CreditApprovalStatus (NotRequired/PendingApproval/Approved/Rejected)'] === 'PendingApproval' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-600'}`}>
                                    ...
                                    {payment['PaymentStatus (Pending/Partial/Paid)'] !== 'Paid' && (
                                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                                            ...
                                            }} className="bg-green-600 text-white font-bold px-4 py-2 rounded-lg shadow-sm hover:bg-green-700 transition-colors">
                                                Mark as Fully Paid
                                            </button>
                                        </div>
                                    )}
            ...
            {isOrderModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-in fade-in">
                    <form onSubmit={handleOrdSubmit} className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95">
                        <div className="px-6 py-4 border-b border-green-100 flex justify-between items-center bg-green-50">
                            <h2 className="text-base font-bold text-green-900 flex items-center gap-2"><ShoppingCart size={18} /> Convert to Order</h2>
                            <button type="button" onClick={() => setIsOrderModalOpen(false)} className="text-gray-400 hover:text-gray-700"><X size={20} /></button>
                        </div>
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Product Ordered *</label>
                                <select value={ordForm.ProductOrdered} onChange={e => setOrdForm({ ...ordForm, ProductOrdered: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded focus:ring-primary focus:border-primary" required>
                                    <option value="">Select Product...</option>
                                    <option value="AAC Blocks">AAC Blocks</option>
                                    <option value="Citabond Mortar">Citabond Mortar</option>
                                    <option value="Kavach Plaster">Kavach Plaster</option>
                                </select>
                            </div>
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    <label className="block text-sm font-bold text-gray-700 mb-1">Quantity *</label>
                                    <input type="number" required value={ordForm.OrderQuantity} onChange={e => setOrdForm({ ...ordForm, OrderQuantity: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded" />
                                </div>
                                <div className="flex-1">
                                    <label className="block text-sm font-bold text-gray-700 mb-1">Unit *</label>
                                    <select value={ordForm.Unit} onChange={e => setOrdForm({ ...ordForm, Unit: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded">
                                        <option value="Cubic Meters">m³</option>
                                        <option value="Tons">Tons</option>
                                        <option value="Bags">Bags</option>
                                    </select>
                            ...
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    ...
                                    <input type="date" required value={ordForm.OrderDate} onChange={e => setOrdForm({ ...ordForm, OrderDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded" />
                                </div>
                                <div className="flex-1">
                                    <label className="block text-sm font-bold text-gray-700 mb-1">Dispatch Scheduled</label>
                                    <input type="date" value={ordForm.DispatchSchedule} onChange={e => setOrdForm({ ...ordForm, DispatchSchedule: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded" />
                                </div>
                            </div>
                        </div>
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-2">
                            ...
                            <button type="submit" className="px-6 py-2 bg-green-600 text-white rounded text-sm font-bold hover:bg-green-700 shadow-sm flex items-center gap-2"><CheckCircle2 size={16} /> Confirm Order</button>
                        </div>
                    </form>
                </div>
            )}
                        ...
                        <div className="p-6 space-y-4 border-b-4 border-blue-500">
                            ...
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    ...
                                    <input type="number" value={payForm.CreditPeriodDays} onChange={e => setPayForm({ ...payForm, CreditPeriodDays: e.target.value })} className={`w-full px-3 py-2 border rounded ${parseInt(payForm.CreditPeriodDays) > thresholdDays ? 'border-orange-400 bg-orange-50' : 'border-gray-300'}`} />
                                    {parseInt(payForm.CreditPeriodDays) > thresholdDays && (
                                        <p className="text-[10px] text-orange-600 font-bold mt-1 leading-tight">Requires Admin Approval (&gt; {thresholdDays} Days)</p>
                                    )}
                                </div>
                                <div className="flex-1">
