...
import { apiCall, getCustomers } from '../api/apiService';
...
export default function RemindersPage() {
    ...
    const handleMarkComplete = async (rId) => {
        try {
            await apiCall('updateReminderStatus', { reminderID: rId, status: 'Completed' });
            setAllReminders(prev => prev.map(r => r.ReminderID === rId ? { ...r, Status: 'Completed' } : r));
            fetchReminders(); // Sync global red badge context
        } catch (e) {
    ...
    const handleDismiss = async (rId) => {
        try {
            await apiCall('updateReminderStatus', { reminderID: rId, status: 'Dismissed' });
            setAllReminders(prev => prev.map(r => r.ReminderID === rId ? { ...r, Status: 'Dismissed' } : r));
            fetchReminders();
        } catch (e) {
    ...
    const getTypeStyle = (type) => {
        switch (type) {
            case 'FollowUp': return { icon: PhoneCall, color: 'text-blue-500', border: 'border-l-blue-500' };
            case 'Payment': return { icon: AlertTriangle, color: 'text-red-500', border: 'border-l-red-500' };
            case 'Dispatch': return { icon: Truck, color: 'text-orange-500', border: 'border-l-orange-500' };
            case 'CrossSell': return { icon: PackageCheck, color: 'text-green-500', border: 'border-l-green-500' };
            default: return { icon: Bell, color: 'text-gray-500', border: 'border-l-gray-300' };
        }
    };
    ...
    const filteredReminders = allReminders.filter(r => {
        ...
        const isCompleted = r.Status === 'Completed' || r.Status === 'Dismissed';
